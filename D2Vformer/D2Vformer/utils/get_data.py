...
def timefeature(dates):
    ...
    dates["hour"] = dates["date"].apply(lambda row: row.hour / 23 - 0.5)  # Hour of the day
    dates["weekday"] = dates["date"].apply(lambda row: row.weekday() / 6 - 0.5)  # Day of the week
    dates["day"] = dates["date"].apply(lambda row: row.day / 30 - 0.5)  # Day of the month
    dates["month"] = dates["date"].apply(lambda row: row.month / 365 - 0.5)  # Day of the year
