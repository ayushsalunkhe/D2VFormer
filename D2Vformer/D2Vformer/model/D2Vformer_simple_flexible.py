...
MODIFICATION: Supports dynamic pred_len at inference time.
This allows ONE trained checkpoint to forecast multiple horizons (48h, 96h, 192h, 336h)
without retraining.
'''

class D2Vformer_simple_flexible(nn.Module):
    """
    Flexible inference variant of D2Vformer_simple.

    Key difference: pred_len can be passed as an argument to forward(),
    enabling variable-horizon forecasting with a single trained model.
    """
    def __init__(self, configs):
        super(D2Vformer_simple_flexible, self).__init__()
        # Changing variable names
        self.configs=configs
        self.seq_len = configs.seq_len
        self.label_len = configs.label_len
        self.default_pred_len = configs.pred_len  # Store default, allow dynamic override
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mode, pred_len=None):
        """
        Forward pass with optional dynamic pred_len.

        Args:
            x_enc: Historical data [B, seq_len, D]
            x_mark_enc: Historical timestamps [B, seq_len, 27]
            x_dec: Decoder input [B, label_len+pred_len, D]
            x_mark_dec: Future timestamps [B, label_len+pred_len, 27]
            mode: 'train' or 'test'
            pred_len: Optional override for prediction length. If None, uses default_pred_len.

        Returns:
            prediction: [B, pred_len, D]
        """
        # Use dynamic pred_len if provided, otherwise fall back to default
        if pred_len is None:
        ...
        # MODIFICATION: Use dynamic pred_len instead of self.pred_len
        ...
        # MODIFICATION: Use dynamic pred_len for slicing
        ...
        return prediction # [B, pred_len, D]
