...
class Fusion_Block_s(nn.Module):
    ...
    def forward(self, x, x_date, y_date, mode):
        ...
        # y is (B, D, O) — shapes that can never broadcast for any config. The intended
        # residual is around the conv feed-forward on the attention output V (B, D, O).
...
# Compatibility alias: model/D2Vformer.py imports Fusion_Block, which was renamed
# in the released code. Fusion_Block_s is the attention-based fusion described in the paper.
