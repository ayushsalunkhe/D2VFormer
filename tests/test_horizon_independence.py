...
class PureD2Vformer(nn.Module):
    """
    Exact mathematical implementation of D2Vformer as specified in arXiv:2409.11024v1 (Eq. 1-10).
    Completely horizon-independent: zero trainable parameters depend on output horizon O.
    """
    def __init__(self, c_in: int, seq_len: int, d_model: int = 128, d_ff: int = 256, k_freq: int = 16, dropout: float = 0.05):
        super().__init__()
...
def run_horizon_independence_test():
    torch.manual_seed(42)
    B, L, D, M = 4, 96, 7, 4
    horizons = [24, 48, 96, 192, 336, 720]
    
    print("=" * 80)
    print("HORIZON-INDEPENDENCE VALIDATION TEST: PureD2Vformer")
    print("=" * 80)
    
    # Instantiate single model
    model = PureD2Vformer(c_in=D, seq_len=L, d_model=128, d_ff=256, k_freq=16, dropout=0.05)
    model.eval()
    
    # Baseline parameter count
    base_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model instantiated with L={L}, D={D}, d_model=128, k_freq=16")
    print(f"Total Trainable Parameters: {base_params:,}\n")
    
    x = torch.randn(B, L, D)
    xm = torch.randn(B, L, M)
    
    param_counts = {}
    
    for O in horizons:
        ym = torch.randn(B, O, M)
        with torch.no_grad():
            y_pred, A = model(x, xm, ym)
            
        current_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        param_counts[O] = current_params
        
        # Assertions
        assert y_pred.shape == (B, O, D), f"Expected shape {(B, O, D)}, got {y_pred.shape}"
        assert A.shape == (B, 128, O, L), f"Expected attention shape {(B, 128, O, L)}, got {A.shape}"
        assert current_params == base_params, f"Parameter count changed for O={O}!"
        
        print(f"Test O = {O:3d} | Output: {list(y_pred.shape)} | Attention: {list(A.shape)} | Params: {current_params:,} | Status: PASSED")
        
    print("\n" + "-" * 80)
    print("PARAMETER COUNT CONSISTENCY CHECK:")
    all_equal = all(count == base_params for count in param_counts.values())
    print(f"Params(O=24) = Params(O=48) = Params(O=96) = Params(O=192) = Params(O=336) = Params(O=720): {all_equal}")
    print("-" * 80)
    
    # Verify backward pass / gradient flow
    print("Verifying Backward Pass and Optimizer Update at O_train = 48...")
    model.train()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    target = torch.randn(B, 48, D)
    ym_train = torch.randn(B, 48, M)
    
    optimizer.zero_grad()
    pred_train, _ = model(x, xm, ym_train)
    loss = nn.MSELoss()(pred_train, target)
    loss.backward()
    optimizer.step()
    print(f"Backward pass succeeded. Training Loss: {loss.item():.6f}")
    print("=" * 80)
    print("ALL UNIT TESTS COMPLETED SUCCESSFULLY WITH ZERO ERRORS.")

if __name__ == '__main__':
    run_horizon_independence_test()
