...
def extract_standard_time_features(df: pd.DataFrame) -> np.ndarray:
    """
    Extracts standard 4 calendar features: [hour, weekday, day, month].
...
class TimeSeriesWindowDataset(Dataset):
    ...
    Constructs exact lookback (seq_len = L) and target forecast horizon (pred_len = O)
...
def get_data_loaders(dataset_name: str, seq_len: int = 96, pred_len: int = 48, batch_size: int = 64, data_root: str = './datasets'):
    """
    Loads and normalizes time series datasets with strict train/val/test splits (60% / 20% / 20%).
    Normalization statistics (mean and std) are calculated strictly on the training set.
    """
    ...
    # Standardization: mean & std on train split only
